...
def test_remove_out_of_range(cleaner):
    df = pd.DataFrame({
...
def test_normalizer_strips_and_title_cases(normalizer):
    df = pd.DataFrame({
...
def test_add_campana_agricola(enricher):
    df = pd.DataFrame({
        "anio": [2023, 2023, 2024],
        "mes": [9, 3, 1],
    })
...
def test_calculate_rendimiento(enricher):
    df = pd.DataFrame({
