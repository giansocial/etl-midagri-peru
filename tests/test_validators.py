...
def test_clean_data_high_score(clean_df):
...
def test_detects_nulls(dirty_df):
...
def test_missing_required_columns(clean_df):
...
def test_quality_score_decreases_with_issues(dirty_df):
